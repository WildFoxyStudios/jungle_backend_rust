...
impl PaymentGateway for BankTransferGateway {
    ...
    async fn create_session(&self, params: PaymentParams) -> Result<PaymentSession, PaymentError> {
        ...
        let ref_code = format!("BT-{}", uuid::Uuid::new_v4().to_string().split('-').next().unwrap_or("0000"));

        let instructions = format!(
            "Transfer {} {} to:\nBank: {}\nAccount: {}\nRouting: {}\nReference: {}\n\nInclude the reference code in your transfer description.",
            params.amount, params.currency.to_uppercase(), bank_name, account_number, routing, ref_code
        );
        ...
        Ok(PaymentSession {
            ...
            redirect_url: format!("{}?instructions={}", params.return_url, urlencoding_simple(&instructions)),
            provider_ref: Some(ref_code),
        })
    }

    async fn verify_payment(&self, reference: &str) -> Result<PaymentStatus, PaymentError> {
        tracing::info!(reference, "Bank transfer verification — requires admin confirmation");
        Ok(PaymentStatus {
    ...
    async fn handle_webhook(&self, _payload: &[u8], _signature: &str) -> Result<WebhookEvent, PaymentError> {
